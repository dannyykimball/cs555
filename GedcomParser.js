...
function output(text) {
  ...
  if (textArr.includes("INDI")) {
    ...
    if (TAGS.includes(textArr[1])) valid = "Y";
    op = op.concat(textArr[0] + "|" + textArr[1] + "|" + valid);
    textArr.splice(1, 1);
  }
  if (textArr.length > 1) op = op.concat("|");
  ...
  if (op.includes("INDI")) {
    prevID = textArr[1].replace("@", "").replace("@", "");
  }
  ...
  if (op.includes("SURN")) {
    hashmap.set(prevID, prevFirst + " " + textArr[1]);
    console.log(hashmap.get(prevID));
    console.log(hashmap);
  }
}
